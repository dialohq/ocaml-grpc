...
module Make_net (Client : Client) :
  ...
  *)
    ...
    let stream_error, stream_error_resolve = Eio.Promise.create () in
    let peek_error () =
      let open Eio.Promise in
      match (peek stream_error, peek Client.connection_error) with
    ...
    let error_handler error =
      if Eio.Promise.is_resolved reader_a then
        Eio.Promise.resolve stream_error_resolve error
    ...
    let writer =
      {
        Grpc_client_eio.Net.write =
          (fun input ->
            if
              Eio.Promise.is_resolved stream_error
              || Eio.Promise.is_resolved Client.connection_error
            then false
            else (
...
module Expert = struct
  let create_with_socket ~sw ~(socket : _ Eio.Net.stream_socket) ~host ~scheme :
        ...
        `Stop_daemon);
    {
      ...
      connection_error = (fun () -> Eio.Promise.peek connection_error);
    }
