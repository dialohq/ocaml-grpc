...
module type SERVERDATA = sig
  val features : feature_list
  val clock : float Eio.Time.clock_ty Eio.Resource.t
end

module RouteGuideServerImplementation (ServerData : SERVERDATA) = struct
  open ServerData
...
let () =
  ...
  let module ServerData : SERVERDATA = struct
    let features = features
    let clock = env#clock
  end in
  Route_guide_server.serve ~net:env#net
    ~on_error:(fun exn -> Eio.traceln "%s" (Printexc.to_string exn))
    port
    (module RouteGuideServerImplementation (ServerData))
